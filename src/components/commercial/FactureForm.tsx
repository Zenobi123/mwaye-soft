...
interface Props {
  clients: Client[];
  onSubmit: (v: { numero: string; client_id: string; date_echeance?: string; montant_total: number; lignes: Ligne[] }) => void;
  isPending: boolean;
  nextNumero: string;
}

export function FactureForm({ clients, onSubmit, isPending, nextNumero }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSubmit({ numero: nextNumero, client_id: clientId, date_echeance: dateEcheance || undefined, montant_total: total, lignes });
    setLignes([{ description: "", quantite: 1, prix_unitaire: 0, montant: 0 }]);
    setClientId(""); setDateEcheance("");
    setOpen(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle>Facture {nextNumero}</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div><Label>Échéance</Label><Input type="date" value={dateEcheance} onChange={e => setDateEcheance(e.target.value)} /></div>
          </div>
          ...
          <Button type="submit" className="w-full" disabled={isPending || !clientId}>Créer la facture</Button>
        </form>
      </DialogContent>
    </Dialog>
  );
